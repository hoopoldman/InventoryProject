...
public class Mystore implements Store{
    ...
    public void store_item(){
        ...
        int condition = filter_Item(temp);  //필터 함수 만들기, count를 반환하여 condition에 저장
        switch(condition){
            case 0:
                inventory.add(temp); //temp 를 리스트에 넣어준다. 왜 바로 넣어주냐? 사용자가 다 입력한거임!
                break;
            case 1:
                Item item1 = new Item(name, num, price, site);
                inventory.add(item1);
                break;
            case 2:
                Item item2 = new Item(name, num, price);
                break;
            case 3:
                Item item3 = new Item(name, num);
                break;
            default:
                String ch;
                System.out.println("제품 이름과 재고는 저장해주세요!");
                System.out.println("다시 재고를 저장하시겠습니까? (y/n): ");
                ch = scanner.next();
                if(ch.equals("y")){
                    store_item();
                }else {
